...
def get_existing_distance_origins(mode, destination):
  with daft_types.get_db_connection(for_type=daft_types.Distance) as conn:
    ...
    cur = conn.execute(
        'SELECT origin FROM distance WHERE mode=? AND destination=?;',
        (mode, destination))
...
def load_distances_to_destination(client, mode, destination):
  ...
  arrival_time=(now + timedelta(days=14 - now.weekday())).replace(
  ...
  with daft_types.get_db_connection(for_type=daft_types.Listing) as conn:
    ...
    while True:
      ...
      logging.info('Querying routes for listings without data: %d out of %d',
      ...
      if matrix['status'] != 'OK':
        raise Exception(
          'distance_matrix returned invalid status: %s' % matrix['status'])
